...
\providecommand{\tblnote}[1]{\par\vspace{3pt}{\footnotesize\noindent #1\par}}
...
CCM2 and LIMA1 only; MR-PRESSO outlier correction left the two strongest
protein-level signals intact and larger in magnitude) indicate that the discordance is a property of the evidence
...
variant only; it is the estimate used for the protein-layer
classification reported in the Results, and it is the estimate given in the
protein-layer coverage table. The number of cis-pQTL instruments listed for a
gene in that table is the number of variants available at that locus, not the
number that contributes to the Wald ratio. The IVW estimate uses all available
cis-pQTL instruments and, with its sensitivity analyses, is reported in the
Results. Wherever the multi-instrument estimate
...
2025, 41 genes from a multi-tissue transcriptome-wide association
study (TWAS)~\cite{hazelwood2025multi}; Chen et
...
\subsection{A saturated genetic layer: 75 genes and 43 colocalizing probe--gene pairs}

The overall study design and the two evidence layers are summarized in
Fig.~\ref{fig:design}, encompassing eight analytical phases from SMR discovery
...
measurements, nine have cis-pQTL instruments, and one colocalizes with CRC risk. The limiting
...
\subsection{Convergent evidence at CCM2 and a concordant tissue layer}
...
\caption{Convergence at CCM2 and GTEx Colon Transverse SMR sensitivity analysis.}
...
\subsection{Tractability and safety of the six core Tier~1 candidates}
...
%==========================================================================
\subsection{Overlap with prior CRC target-discovery studies is only partial}

We compared our 69 unique gene symbols with six prior CRC genetic studies, to
place the present gene set in the context of the existing literature
(Fig.~\ref{fig:competitor}A and Supplementary Table~S9). Overlap was minimal
with the four smaller studies: Hong et al.~2025 (9
genes)~\cite{hong2025integrative}, Wang et al.~2025 (4
genes)~\cite{wang2025discovoncol}, Xia and Wang
(6 genes)~\cite{xia2025bmccancer} and Tian et al.~2025 (8
genes)~\cite{tian2025dbi} shared at most three gene symbols with our set. Overlap was higher but still incomplete with
the two largest prior studies: 44.9\% with Chen et
al.~2024~\cite{chen2024integrative} (289 resolvable gene
symbols from a 254,000-individual GWAS with multi-tissue eQTL/mQTL) and 17.4\%
with Hazelwood 2025~\cite{hazelwood2025multi} (41 genes from a multi-tissue
TWAS). Of our 69 symbols, 34
(49\%) were absent from both prior lists (Fig.~\ref{fig:competitor}B).

At the level of independent loci, 23 of our 37 loci (62.2\%) contained at least
one gene also reported by Chen et al.~2024, whereas 14 loci (37.8\%) were not
reported by any prior study.
...
studies (Hong, Wang, Xia \& Wang, Tian) showed $\leq$3 gene overlap;
...
The degree of overlap tracked the analytical approach rather than the
disease. The higher overlap with Chen 2024 is expected given that both Chen 2024 and the present
...
This is, to our knowledge, the first systematic documentation of
eQTL/pQTL
...
We interpret these data as indicating that the CRC susceptibility
...
reviewed by the author, tested and validated, and every AI-suggested edit was
critically evaluated before acceptance. OpenAI Codex (a GPT-5-class coding agent) was accessed through the Codex desktop application, and Claude (Anthropic) through its web interface; both were used between August 2026 and September 2026. No generative AI tool was used to
