...
detectable in this section (11--57\% of spots; Supplementary Fig.~\ref{fig:S8}),
...
sections that were not analysed here) and a single single-cell reference
($n = 50{,}214$ cells), so the observations above describe one tumour and were not tested
...
spatial--microenvironment associations are unadjusted for multiple comparisons, and the
TMBIM1--meCAF correlation noted above explains less than 1\% of the variance even though it
is significant. We therefore treat the single-cell and spatial results as supporting
