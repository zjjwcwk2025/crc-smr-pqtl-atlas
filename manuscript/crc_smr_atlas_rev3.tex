% CRC Multi-Omics Drug-Target Atlas (v5)
...
\usepackage{xr}
\externaldocument{crc_smr_supplementary}
...
annotation into a single target-prioritization pipeline.
...
TNF-pathway genes analysed outside the Bonferroni set), only nine had cis-pQTL instruments
...
935,000 deaths annually worldwide, ranking third in incidence and second in
mortality among all cancers~\cite{sung2021global, bray2024global, siegel2023colorectal}.  Large-scale GWAS have
...
saturated: 15{,}582 gene-level SMR tests yield 43 colocalising probe--gene pairs out of 76.
The protein layer (centre) collapses: only 15 of the 62 protein-coding candidates have plasma
...
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=\linewidth]{Fig1a_study_design.pdf}
...
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=\linewidth]{Fig1b_coloc_pph4.pdf}
...
\label{fig:coloc}
\end{subfigure}
\caption{Study design and genetic-layer colocalization screen (continued).}
\end{figure}
...
10^{-6}$; 0.05/15,582). The genomic inflation factor was $\lambda = 1.600$, consistent with LD-coupled cis-eQTL SMR statistics~\cite{wainberg2019} (Supplementary Fig.~\ref{fig:S1}).

The top 10 SMR associations are listed in Supplementary Table~\ref{tab:smr_top10}. The
...
validation (Supplementary Table~\ref{tab:protein_coding}). Seven non-coding genes lacked a gene
...
total, each with single-SNP credible sets (PIP = 1.0; Supplementary Table~\ref{tab:susie_sensitivity}).
...
summarized in Table~\ref{tab:tier1} (UTP11, BMP2, PNKD, LAMC1, TMBIM1, GPBAR1). Regional association plots for all Tier 1 loci are provided in Supplementary Fig.~\ref{fig:S23}. Among the
...
\input{manuscript/tables/Table1_tier1_genes.tex}
...
the method/settings, not evidence establishing a single causal variant (Supplementary Fig.~\ref{fig:S3}).
...
Table~\ref{tab:susie_sensitivity}).
...
expression (Supplementary Table~\ref{tab:smr_top10} lists $b_{\text{SMR}}$ and $p_{\text{HEIDI}}$
...
ratios for the six genes are shown in Supplementary Fig.~\ref{fig:S2}).
...
are flagged in Supplementary Table~\ref{tab:protein_coding} and Supplementary Fig.~\ref{fig:S6}. BMP2 had no HEIDI result; the remaining 12
passed.
...
NCF2, TET2, TNF). MR analysis identified CDKN1A (Wald $p = 0.020$) and TNF
(Wald $p = 5.0\times10^{-4}$) as significant, with effects in the risk-increasing
direction for CDKN1A (Wald $b = +0.130$) and protective direction for TNF
(Wald $b = -0.295$).  However, none of the 5 genes showed pQTL--GWAS colocalization (max PPH4 = 0.275 for TNF; Supplementary Table~\ref{tab:ukbppp_pqtl_coloc}; Supplementary Fig.~\ref{fig:S4}).
...
had significant cis-pQTL instruments (at $p < 5\times10^{-8}$):
LIMA1 (70 instruments, Wald $b = +0.858$, $p = 4.29\times10^{-7}$), CCM2 (94 instruments,
Wald $b = -0.766$, $p = 6.28\times10^{-6}$), and STAT6 (61 instruments, Wald $b = -0.029$, $p =
3.86\times10^{-5}$) were significant, while TNFRSF1A (Wald $b = -0.070$, $p = 0.349$) and
TNFRSF1B (Wald $b = -0.035$, $p = 0.614$) were not. The seven remaining deCODE-only genes had
...
Supplementary Table~\ref{tab:decode_pqtl_coloc}, Supplementary Fig.~\ref{fig:S5}). This makes CCM2 the only gene in our study with
...
(Wald $p = 0.349$). TNFRSF1B showed neither colocalization (PPH4 = 0.0084)
nor a distinct pQTL signal (PPH3 = 0.287), consistent with its
non-significant pQTL association (Wald $p = 0.614$).

\input{manuscript/tables/Table2_pqtl_coverage.tex}

Formal instrument-strength and pleiotropy diagnostics for the nine genes with
cis-pQTL instruments are reported in Supplementary Figs.~\ref{fig:S20}--\ref{fig:S21} and
Supplementary Figs.~\ref{fig:S24}--\ref{fig:S27}. Every instrument in all 17 gene--dataset
combinations had an F statistic above the conventional weak-instrument threshold
(minimum F = 18.2; median F per gene 30.4--326.3). Cochran's Q indicated
substantial heterogeneity for all five genes with multi-instrument IVW estimates
(CCM2 Q = 770.7, df = 93, p = 2.8e-107; LIMA1 Q = 575.7, df = 69, p = 5.3e-81;
STAT6 Q = 177.0, df = 60, p = 1.7e-13; CDKN1A Q = 687.1, df = 227, p = 8.8e-48;
TNF Q = 9683.2, df = 2287, p < 1e-300). MR-Egger intercepts were non-significant
for CCM2 (b = 0.013, p = 0.55) and LIMA1 (b = 0.074, p = 0.11), the two genes
with colocalizing or near-colocalizing protein-level support, but were significant
for STAT6 (b = 0.021, p = 3.0e-12), CDKN1A (b = -0.012, p = 1.8e-4) and TNF
(b = -0.012, p < 1e-300), indicating directional pleiotropy in the latter three.
MR-PRESSO detected pleiotropy in all four genes for which it was computationally
feasible (global test p < 2e-4 in each, the resolution limit of the 5,000-draw
null distribution) and flagged 5 of 61 (STAT6), 10 of 70 (LIMA1), 15 of 94 (CCM2)
and 22 of 228 (CDKN1A) instruments as outliers; the distortion test was
significant for LIMA1, CCM2 and CDKN1A (p < 2e-4) but not for STAT6 (p = 0.085),
so in the first three the outliers do shift the point estimate. Removing them did
not change the direction of any estimate: the two significant protein-level
signals became stronger (CCM2, b = -0.52 raw versus -0.76 outlier-corrected,
p = 1.9e-57; LIMA1, 0.88 versus 1.08, p = 8.1e-37), whereas STAT6 stayed null
(0.004, p = 0.67) and CDKN1A was essentially unchanged (-0.13 versus -0.10,
p = 5.9e-20). Because cis-pQTL instruments come from a single locus and remain
partly correlated, the MR-PRESSO global test is anti-conservative here, and we
therefore read it as a sensitivity check rather than as evidence against a causal
effect. Instrument strength alone therefore does not explain the pattern; only
CCM2 and LIMA1 carry a coherent protein-level signal
(Table~\ref{tab:mr_sensitivity}).

\input{manuscript/tables/Table3_mr_sensitivity.tex}
...
proteomic panels (deCODE and UKB-PPP); adding the two TNF-pathway genes analysed outside the
Bonferroni set (TNFRSF1A, TNFRSF1B) gives the 17-gene protein-layer analysis set. Eight of the
...
significant (B), and 12 without significant pQTL signal (C).}
...
\end{subfigure}
\caption{The protein layer does not corroborate the genetic layer (continued).}
\end{figure}
...
(Supplementary Fig.~\ref{fig:S7}) and broad detection in CRC tissue (57\% of
3{,}493 Visium spots; Supplementary Fig.~\ref{fig:S8}). In contrast,
GPBAR1 was detected in only 1 of 3,493 Visium spots (0.03\%). Partitioning
Visium spots into $k=6$ spatial microenvironment (ME) zones showed no zone-restricted expression: mean expression of the six core
candidates varied only 1.25--1.59-fold across the six zones, and UTP11, BMP2,
LAMC1 and TMBIM1 each reached their lowest mean expression in the stromal ME6
zone (PNKD: ME6 second lowest; Supplementary Fig.~\ref{fig:S14};
Supplementary Table~\ref{tab:me_zones}). Pairwise spatial cross-correlation statistics for the
chr2:219Mb locus are provided in Supplementary Table~\ref{tab:spatial_crosscorr}.

The tissue layer can localize a candidate gene within the tumour but cannot
...
significant in both tissues showed identical direction. However, 5 of the 6 prioritized non-MHC Tier 1 genes lacked detectable colon cis-eQTLs, reflecting the power limitation of tissue-specific eQTL datasets (Fig.~\ref{fig:tissue_layer}). Forest plots for the FinnGen directional-consistency analysis are provided in Supplementary Fig.~\ref{fig:S22}.
...
\end{subfigure}
\caption{GTEx Colon Transverse SMR sensitivity analysis (continued).}
\end{figure}
...
and PNKD (Tbio, limited functional annotation with no drug discovery infrastructure; Supplementary Fig.~\ref{fig:S18}).
...
(Table~\ref{tab:tier1}; Supplementary Table~\ref{tab:phewas_safety}). BMP2 showed the narrowest risk
...
(Fig.~\ref{fig:phewas}; Supplementary Fig.~\ref{fig:S19}). Because the composite
safety score in Supplementary Table~\ref{tab:phewas_safety} and Supplementary Fig.~\ref{fig:S19} is
...
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=\linewidth]{Fig4b_phewas_safety.pdf}
...
Table~\ref{tab:phewas_safety} and Supplementary Fig.~\ref{fig:S19}; it is dominated by the number of genetic
...
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=\linewidth]{Fig4a_drug_repurposing.pdf}
...
\label{fig:drug}
\end{subfigure}
\caption{Tractability and safety of the six core non-MHC Tier 1 candidates (continued).}
\end{figure}
...
The directional discordance observed for CDKN1A (eQTL SMR protective,
$b = -0.288$; pQTL MR risk-increasing, $b = +0.130$) merits particular
attention. This bidirectional signal may reflect the dual role of CDKN1A/p21
as both a cell-cycle inhibitor (tumor suppressive) and an anti-apoptotic
regulator (pro-survival in stressed cells). The pQTL signal was
driven by the rs1801270 (C31S) missense variant, which alters p21 protein
stability, potentially explaining how the same gene can exhibit opposite
transcript- and protein-level effects. Discordant transcript- and
protein-level effects from one locus therefore require multi-level evidence
before drug-target prioritization.
...
largest prior CRC genetic study (254K individuals, 290 genes), and
...
that test different biological hypotheses and consequently identify partially non-overlapping gene sets~\cite{chen2024integrative, hazelwood2025multi}. The gene-level comparison underlying Fig.~\ref{fig:competitor} is provided in Supplementary Table~\ref{tab:chen2024_overlap}.
To our knowledge this is the first systematic documentation of eQTL/pQTL
...
\begin{subfigure}{\textwidth}
\centering
\includegraphics[width=\linewidth]{Fig5b_competitor_upset.pdf}
\caption{UpSet plot of shared and unique genes between this study (69
gene symbols), Chen 2024 (290 genes), and Hazelwood 2025 (41 genes).}
\label{fig:competitor_upset}
\end{subfigure}
\caption{The genetic layer is largely saturated: overlap with prior CRC target-discovery studies (continued).}
\end{figure}
...
signals at this region, but neither provided the single-SNP causal resolution
that our SuSiE analysis achieved, separating PNKD, TMBIM1, and GPBAR1 into
independent credible sets. This matters for drug development: the three genes
span a 222 kb region with distinct druggability profiles (Tclin, Tbio, Tchem
respectively), and conflating them would lead to an ambiguous target
prioritization. UTP11 was not reported by any prior CRC study, and GPBAR1 was
...
work is therefore the \textit{analytical resolution} (SuSiE
fine-mapping, eQTL--pQTL colocalization discordance characterization, spatial
transcriptomics) and \textit{target tractability assessment} (IDG druggability
classification, PheWAS, drug repurposing scoring) applied to both shared and
unique loci.
...
SuSiE fine-mapping resolved single-SNP credible sets at each of these loci. BMP2 (bone morphogenetic protein 2) is a member of the
...
factors such as BMI and processed meat intake~\cite{chan2010primary}. BMP2 had
...
lies in analytical resolution (SuSiE fine-mapping producing single-SNP credible
sets at loci where prior coloc-only approaches could not separate causal
signals), multi-omic integration (the first systematic documentation of eQTL/pQTL
colocalization discordance in CRC, with CCM2 as the sole triple-convergent
gene), and biological contextualization (spatial transcriptomics characterizing
tumor microenvironment zones and gene-level spatial independence at the complex
chr2:219Mb locus). The eQTL/pQTL discordance suggests that current proteomic
...
~0.3--0.5, consistent with the observed 1/8 nominal significance. Third, we
...
not be interpreted as regulatory-grade safety assessment. Fifth, we did not
...
Catalog~\cite{buniello2019gwas}. All genomic coordinates are reported
in hg38.
...
variants, and such genes are flagged in Supplementary Table~\ref{tab:protein_coding} and Supplementary Fig.~\ref{fig:S6}. Accordingly,
...
transcription start site. Mendelian randomization was performed using
...
al.~2024, 290 genes from 254K-individual GWAS + SMR + multi-tissue
...
predicted genes), merged and deduplicated to 290 unique gene symbols. Overlap
was defined as identical or synonymous gene symbols. At the locus level, a v5
...
In accordance with the journal policy on AI-assisted technologies, the authors
disclose that Claude (Anthropic) and Codex (OpenAI) were used for language
...
Nature Communications article supplement (41467\_2024\_47399\_MOESM4\_ESM.xlsx).
